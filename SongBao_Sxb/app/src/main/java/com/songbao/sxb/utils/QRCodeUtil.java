...
public class QRCodeUtil {
	 ...
	 * 生成普通二维码图片
	 */
	public static Bitmap createQRImage(String url, int widthPix, int heightPix, int marginPix) {
		return createQRImage(url, widthPix, heightPix, marginPix, null, null);
	}

	/**
	 * 生成带Logo的二维码图片
	 */
	public static Bitmap createQRImage(String url, int widthPix, int heightPix, int marginPix, Bitmap logoBm) {
		return createQRImage(url, widthPix, heightPix, marginPix, logoBm, null);
	}
	...
	public static Bitmap createQRImage(String url, int widthPix, int heightPix, int marginPix, Bitmap logoBm, Bitmap drop) {
		...
		try {
			...
			// 设置空白边距的宽度
			hints.put(EncodeHintType.MARGIN, marginPix);
			// 图像数据转换，使用了矩阵转换
			...
			if (logoBm != null) {
				// 添加Logo边框
				Bitmap logoFrame = BitmapFactory.decodeResource(AppApplication.getAppContext().getResources(), R.drawable.icon_bg_white);
				if (logoFrame != null) {
	...
	public static boolean createQRImage(Bitmap drop, String content,
		...
		try {
			...
			// 设置空白边距的宽度
	        hints.put(EncodeHintType.MARGIN, marginPix); //default is 4
			// 图像数据转换，使用了矩阵转换
